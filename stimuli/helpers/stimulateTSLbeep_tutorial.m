function [t, stimOn, stimEnd, tmaxT, tminT, resp] = stimulateTSLbeep_tutorial...
    (vars, scr, tslType, tb, np, r, tone, sd, catchT, varargin)
    ...
    while stim_idx < nTSLsteps %keep looping until 4 steps of TSL are complete
        ...
        if stim_idx == 1 %to tmax
            if isempty(varargin) || varargin{1} < tb
                % Display help text
                DrawFormattedText(scr.win, uint8(vars.display.pressButtonText), 'center', 'center', scr.TextColour);
                [~, textTimeOn] = Screen('Flip', scr.win);
