...
model {
  ...
  for(s in 1:S){
    ...
    c1_conf[s] ~ induced_dirichlet([1,1,1]', 0, 2, c0_conf[s], c1_conf[s]);
     ...
     target += lognormal_lpdf(rt[starts[s]:ends[s]]-ndt[s]| a_rt[s]+(b_rt[s]*p[starts[s]:ends[s]])+(b1_rt[s]*tcf[starts[s]:ends[s]]), sigma_rt[s]);
  }
  // estimating phs and rt
  for(n in 1:N){
    conf[n] ~ ord_beta_reg(a_conf[S_id[n]]+(b_conf[S_id[n]]*p[n])+(b1_conf[S_id[n]]*tcf[n]), phi_conf[S_id[n]], c0_conf[S_id[n]], c1_conf[S_id[n]]);
  }
}
...
generated quantities{
  ...
  for(n in 1:N){
    ...
    ll_rt[n] = lognormal_lpdf(rt[n]-ndt[S_id[n]]|  a_rt[S_id[n]]+(b_rt[S_id[n]]*p_gq[n])+(b1_rt[S_id[n]]*tcf[n]), sigma_rt[S_id[n]]);
    ll_conf[n] = ord_beta_reg_lpdf(conf[n]|a_conf[S_id[n]]+(b_conf[S_id[n]]*p_gq[n])+(b1_conf[S_id[n]]*tcf[n]), phi_conf[S_id[n]], c0_conf[S_id[n]], c1_conf[S_id[n]]);
