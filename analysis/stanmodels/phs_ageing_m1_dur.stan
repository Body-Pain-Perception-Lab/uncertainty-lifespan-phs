...
data {
  ...
  vector[S] min_rt; //minimum RT of given participant

  vector[N] duration; //input - confidence
...
parameters {
  // phs
  vector[5] mus_phs;
  vector<lower=0>[5] sigmas_phs;
  matrix[5,S] difs_phs;
  
  // rt
  vector[6] mus_rt;
  vector<lower=0>[6] sigmas_rt;
  matrix[6,S] difs_rt;
  
  // confidence
  vector[7] mus_conf;
  vector<lower=0>[7] sigmas_conf;
  matrix[7,S] difs_conf;
  
  // age
  vector[3] mus_age; //age coefficients for phs, rt, ndt and conf
}
...
transformed parameters {
  ...
  // phs
  vector[S] a_phs = to_vector(mus_phs[1]+(difs_phs[1,])*sigmas_phs[1]);
  
  vector[S] b_phs = exp(to_vector(mus_phs[2]+(difs_phs[2,])*sigmas_phs[2]));
  ...
  vector[S] dur_phs = (to_vector(mus_phs[5]+(difs_phs[5,])*sigmas_phs[5]));
  
  
  // rt
  vector[S] a_rt = to_vector(mus_rt[1]+(difs_rt[1,])*sigmas_rt[1])+mus_age[2].*age_z;
  vector[S] b_rt = to_vector(mus_rt[2]+(difs_rt[2,])*sigmas_rt[2]); //by phs
  vector[S] b1_rt = to_vector(mus_rt[3]+(difs_rt[3,])*sigmas_rt[3]); //by tcf
  vector[S] sigma_rt = to_vector(exp(mus_rt[4]+(difs_rt[4,])*sigmas_rt[4]));
  vector[S] ndt_uc = to_vector(mus_rt[5]+(difs_rt[5,])*sigmas_rt[5]); //non-decision time
  vector[S] ndt = 0.2+inv_logit(ndt_uc).*(min_rt-0.2); //constraining ndt
  vector[S] dur_rt = to_vector(mus_rt[6]+(difs_rt[6,])*sigmas_rt[6]);
  
  
  // confidence
  vector[S] a_conf = to_vector(mus_conf[1]+(difs_conf[1,])*sigmas_conf[1])+mus_age[3].*age_z;
  vector[S] b_conf = to_vector(mus_conf[2]+(difs_conf[2,])*sigmas_conf[2]); //by phs
  vector[S] b1_conf = to_vector(mus_conf[3]+(difs_conf[3,])*sigmas_conf[3]); //by tcf
  ...
  vector[S] dur_conf = to_vector(mus_conf[7]+(difs_conf[7,])*sigmas_conf[7]);
...
model {
  ...
  mus_phs[5] ~ normal(0,1); //lapse
  sigmas_phs[5] ~ normal(0,1);
  ...
  mus_rt[6] ~ normal(0,1); //non-decision time
  sigmas_rt[6] ~ normal(0,1);
  ...
  mus_conf[7] ~ normal(0,1); //cut1
  sigmas_conf[7] ~ normal(0,1);
  ...
  for(s in 1:S){
    ...
    p[starts[s]:ends[s]] = g_phs[s]+(1-g_phs[s]-l_phs[s])*(inv_logit(b_phs[s]*(tcf[starts[s]:ends[s]]-exp(a_phs[s]+mus_age[1].*age_z[s] + duration[starts[s]:ends[s]] * dur_phs[s])))); //log_probit function
    phs[starts[s]:ends[s]] ~ bernoulli(p[starts[s]:ends[s]]);
    target += lognormal_lpdf(rt[starts[s]:ends[s]]-ndt[s]| a_rt[s]+(b_rt[s]*entropy_vec(p[starts[s]:ends[s]]))+(b1_rt[s]*tcf[starts[s]:ends[s]] + duration[starts[s]:ends[s]] * dur_rt[s]), sigma_rt[s]);
  ...
  for(n in 1:N){
      conf[n] ~ ord_beta_reg(a_conf[S_id[n]]+(b_conf[S_id[n]]*entropy(p[n]))+(b1_conf[S_id[n]]*tcf[n]) + duration[n] * dur_conf[S_id[n]], phi_conf[S_id[n]], c0_conf[S_id[n]], c1_conf[S_id[n]]);
...
generated quantities{
  ...
  vector[S]  cutzero = c0_conf;
  vector[S]  cutone = c0_conf + exp(c1_conf);
  
  for(n in 1:N){
    p_gq[n] = g_phs[S_id[n]]+(1-g_phs[S_id[n]]-l_phs[S_id[n]])*(inv_logit(b_phs[S_id[n]]*(tcf[n]-exp(a_phs[S_id[n]]+mus_age[1].*age_z[S_id[n]] + duration[n] * dur_phs[S_id[n]])))); //log_probit function
  
    ll_bin[n] = bernoulli_lpmf(phs[n]|p_gq[n]);

    ll_rt[n] = lognormal_lpdf(rt[n]-ndt[S_id[n]]|a_rt[S_id[n]]+(b_rt[S_id[n]]*entropy(p_gq[n]))+(b1_rt[S_id[n]]*tcf[n])+ duration[n] * dur_rt[S_id[n]], sigma_rt[S_id[n]]);

    ll_conf[n] = ord_beta_reg_lpdf(conf[n]|a_conf[S_id[n]]+(b_conf[S_id[n]]*entropy(p_gq[n]))+(b1_conf[S_id[n]]*tcf[n])+ duration[n] * dur_conf[S_id[n]], phi_conf[S_id[n]], c0_conf[S_id[n]], c1_conf[S_id[n]]);
