...
model {
  ...
  mus_conf[6] ~ normal(2,.25); //cut1
    ...
    target += lognormal_lpdf(rt - ndt| a_rt+(b_rt*(p))+(b1_rt*tcf), sigma_rt);

  // estimating phs and rt
  for(n in 1:N){
      conf[n] ~ ord_beta_reg(a_conf+(b_conf*(p[n]))+(b1_conf*tcf[n]), phi_conf, c0_conf, c1_conf);
...
generated quantities{
  ...
  for(n in 1:N){
    ...
    ll_rt[n] = lognormal_lpdf(rt[n]-ndt|a_rt+(b_rt*(p_gq[n]))+(b1_rt*tcf[n]), sigma_rt);

    ll_conf[n] = ord_beta_reg_lpdf(conf[n]|a_conf+(b_conf*(p_gq[n]))+(b1_conf*tcf[n]), phi_conf, c0_conf, c1_conf);
